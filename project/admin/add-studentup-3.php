...
{ 
  ...
  if(isset($_POST['btn_search'])){
   ...
   $names="";
   $lnames="";
   $sql_s=mysqli_query($con,"SELECT * FROM viewstudent  WHERE (SName='$sname' and Slastname='$slname') and Yname= 2 ");
...
if(isset($_POST['btn_saveth'])){ 
 ...
 $sql_th=mysqli_query($con,"UPDATE `tbl_student` SET `s_active`=3, YID=11  WHERE `SID`='$id_th'");
 if($sql_th){
   $sql_t=mysqli_query($con,"UPDATE `tbl_register` SET `SFID`=0,`YID`=11,`Rdate`='$date',`R_active`=3  WHERE `SID`='$id_th'");
   
   $sectecl=mysqli_query($con,"SELECT * from tbl_register where SID='$id_th'");
        if($row=mysqli_fetch_array($sectecl)){
           $RID=$row['RID'];
        }
          $inserpay=mysqli_query($con,"INSERT INTO `tbl_paystudent`(`SFID`, `RID`, `SID`, `YID`, `PTstatus`, `PTmoney`, `P_datae`, `py_active`) VALUES (0,'$RID','$id_th',11,0,0,'$date',3)");

    $msg="ບັນທືກ ສຳເລັດແລ້ວ";

  header("refresh:3; url=add-studentup-3.php");
}
else{
...
<body class="fixed-left">
  <div id="wrapper">
            ...
            <div class="card-box"  style="background: #E5F1FB;">
              <nav class="navbar navbar-expand-lg navbar-light bg-light">
                ...
                <form class="form-inline my-2 my-lg-1">
                  ...
                  <a href="add-studentup-3.php" class="btn btn-secondary">Refesrch</a>
                  <br><br>
                  <div>
                   <?php if($error){ ?>
              ...
              while ($row=mysqli_fetch_array($sql_r)) {   
               ...
               <tbody>
                <tr> 
                  ...
                  <td>  <a href="openthree.php?id=<?php echo $id; ?>" class="btn btn-warning btn-sm"><i class="fa fa-unlock"></i>ໄດ້ຮຽນ</a></td>
                </tr>
    ...
    if(isset($_POST['search'])){
      ...
      $sql=mysqli_query($con,"SELECT * FROM `viewstudenthree` WHERE DName='$deparment' and STname='$times' AND S_active=3");
      ...
      <div  class="container">    
        <div class="modal fade " id="addModal<?php echo $code;?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-xl">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">ລົງທະບຽນປີ 3</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <?php 
                $sql_add=mysqli_query($con,"SELECT `SID`, `Sdate`, `SName`, `Slastname`, `DName`, `STname`, `Yname`, `SN_tell` FROM `viewstudentwo` WHERE SID ='$code'");
